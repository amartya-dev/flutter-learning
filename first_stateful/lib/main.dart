...
class _AppWidgetState extends State<AppWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: FlowerWidget(
          imageSrc: _bright
              ? "https://mfiles.alphacoders.com/663/663347.jpg"
...
class _FlowerWidgetState extends State<FlowerWidget> {
  ...
  /** 
  * Fired when Flutter detects that the data
  from another source has changed, 
  * possibly affecting the UI and causing a call
  to ‘build’. 
  * In this case it is when the Theme changes
  (its an InheritedWidget). 
  */
  ...
  @override
  /** 
  * Fired when the widget is reconstructed as its
  widget data has changed, 
  * In this case it is when a new FlowerWidget is
  created with a different 
  * imageSrc.*/
  void didUpdateWidget(Widget oldWidget) {
    debugPrint("_FlowerWidgetState - didUpdateWidget -".hashCode.toString());

    // Flower Image has changed so reset blur
    _blur = 0;
  }
